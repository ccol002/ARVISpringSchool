...
public class Interface {
	...
	{
		// ===BEGIN Verification [P4]
		Integer count=0;
		Iterator<UserInfo> iterator_u = ts.getUsers().iterator();
		while (iterator_u.hasNext()) {
			Iterator<UserAccount> iterator_a = ts.getUserInfo(iterator_u.next().getId()).getAccounts().iterator();
			while (iterator_a.hasNext()) {
				if (iterator_a.next().getAccountNumber().equals(account_number)) count++;
			}
		}
		Verification.assertion(count==1,"P4 violated");
		// ===END 
	...
	{
		Verification.addExternalTransfer(amount);
		Verification.assertion(!Verification.reconciliationNeeded(), "P8 violated");
